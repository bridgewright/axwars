...
description: 소스 GAAP(일반기업회계기준·베트남 VAS·중국 CAS·미국 US GAAP) 시산표를 K-IFRS로 변환해 IFRS 재무제표·전환조정 명세서(기준서 인용)·계정별 상세 차이분석·영향분석을 산출한다. 회계기준 전환 자문, GAAP→IFRS 변환, 상장전환/해외자회사 연결 전환, "이 계정이 IFRS에서 어떻게 바뀌고 수익성·자본에 어떤 영향" 류 요청에 사용. 각 조정의 IFRS 조항 근거는 로컬 코퍼스 원문(verbatim)으로 grounding하며, 코퍼스에 없으면 "큐레이션 요약"으로 명시한다.
---
...
- **`difference_analysis.md`** — 회계사용 상세 보고서: 계정(Layer 1)·조정(Layer 2)마다 **IFRS 근거(로컬 코퍼스 원문 verbatim) + 이전 GAAP·핵심 차이 + 판단 논리 + 분개 파급효과(자산·부채·자본·당기순이익이 얼마 움직이는지)** 를 **단위와 함께** 설명
- `impact_analysis.xlsx`, `result.json`

조항 **포인터**(`ifrs_ref`)는 `gaap_ifrs/data/*.json`의 `basis`에서 오고, 그 **원문은 로컬 코퍼스**(`corpus/`, 트랙 2 MCP와 공유)에서 verbatim으로 조회해 삽입한다(엔진-side, 결정론 — 같은 입력이면 보고서 바이트 동일). 코퍼스가 없거나(standalone 실행) 특정 문단이 미적재면 손으로 쓴 요약을 **"큐레이션 요약 — 코퍼스 원문 미확인"** 으로 라벨해 폴백한다(지어내지 않음). 코퍼스 경로는 `--corpus-dir`로 지정하며 기본은 자동탐색이다.
...
## 판단 기준 / 안전장치
- **지식 = 근거, 계산 = 코드 (분리).** 어느 IFRS 기준·조항을 적용할지(**포인터**)는 데이터 JSON에서, 그 **조항 원문은 코퍼스에서 verbatim**으로 인용하고, **숫자는 결정론적 파이썬**이 계산 → 재현·검증 가능. LLM은 숫자·조항 원문을 생성하지 않는다.
