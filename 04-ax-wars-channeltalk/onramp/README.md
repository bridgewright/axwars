...
`intake`(인터뷰 → `deployment-discovery.json`) → `brief`(→ 두 보고서) 2-스킬 파이프라인.
...
[고객 현업]  ──ElevenLabs 음성 링크/CLI──▶  intake
                                             ...
                                             └ transcript → deployment-discovery.json (validate 게이트)
                                                     │
                                                     ▼  brief
                                             ├▶ deployment-brief.md   (배포 계획서: C레벨 보고서로 에이전트가 직접 집필)
                                             └▶ product-input.prd.md  (본진 PRD: build_reports.py 렌더)
```
- **배포 계획서는 에이전트가 집필**: `brief` 스킬은 `references/deployment-brief-format.md`(두괄식·명사형·각주·7섹션·스코어링·원가)를 따라 discovery로 C레벨 보고서를 쓴다. 목차: Executive Summary → 현황 파악 → 페인포인트(원인 진단) → 도입 에이전트·우선순위(스코어링 테이블) → 기술 고려사항 → 도입 원가·제시가 → 추가 세일즈.
...
- **AI 담당**: 음성 인터뷰 진행(ElevenLabs + Claude), 대화에서 discovery 슬롯 채우기, **배포 계획서를 포맷 레퍼런스대로 집필**(C레벨 보고서 품질), PRD 문장 합성.
- **사람/코드 담당**: 자동화 범위·연동 스코핑 판단은 배포 담당자, 계약 검증(`validate_discovery.py`)과 PRD 렌더(`build_reports.py`)는 **결정적 stdlib 코드**(LLM 비의존). 원가는 `costing-assumptions.md` 가정 × 인터뷰가 캐낸 공수로 산출. AI 저작(서사)과 결정적 계약(스키마·검증)을 분리해 재현성과 품질을 함께 확보.

## 5. 어떻게 검증했나

- **입력→결과 예시**: `samples/무브온/`(1개 회사 4개 그룹 9인 인터뷰) → `deployment-brief.md`(새 포맷 exemplar) + `product-input.prd.md`. 단일 롤 최소 예시는 `skills/intake/assets/sample-discovery.json`.
- **테스트**: `pytest`로 스키마 검증(enum·필수키·경고), PRD 렌더(섹션·인용·태그), transcript 캡처 = 총 18건 통과.
- **예외 처리**: 잘못된 enum/누락 키는 검증에서 걸러 렌더 거부. 미확보 슬롯은 "확인 필요"로 표기.

```bash
# PRD 렌더 데모
python3 skills/brief/scripts/build_reports.py skills/intake/assets/sample-discovery.json --out-dir /tmp/onramp
# 테스트
python3 -m pytest skills/intake/tests skills/brief/tests -q
```
...
- `skills/brief/` — discovery.json → 두 보고서 (deployment-brief는 에이전트 집필, PRD는 렌더)
