...
```
[고객 현업]  ──ElevenLabs 음성 링크/CLI──▶  interview
...
`skills/interview/README.md` 참조. 요약: `uv pip install -r skills/interview/requirements.txt` → `.env`에 `ELEVENLABS_API_KEY` → `setup_agent.py --write-env` → `serve_browser.py`.

## 구조

- `skills/interview/` — 음성 인터뷰 → discovery.json (scout `distillery` 포크)
